setwd('/mnt/hdd/chaozhong/manuscript/fig2/Cite_data/bindSC/')
#library(devtools)
...
gene.use <- intersect(rownames(rna.so),rownames(adt.so))
...
write.table(
  res$u, '../results/raw/bindsc_embed.adt.txt', #args$output_rna,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE
)
write.table(
  res$r, '../results/raw/bindsc_embed.rna.txt', #args$output_atac,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE
)
