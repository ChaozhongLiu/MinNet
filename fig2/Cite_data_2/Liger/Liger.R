setwd('/mnt/hdd/chaozhong/manuscript/fig2/Cite_data_2/Liger/')
...
int.liger <- selectGenes(int.liger, unshared = TRUE, 
...
int.liger <- optimizeALS(int.liger, k=30, 
                         use.unshared = TRUE, rand.seed = rand_seed)
int.liger <- quantile_norm(int.liger, ref_dataset= "rna")
int.liger <- runUMAP(int.liger)
#int.liger <- louvainCluster(int.liger)
...
write.table(
  adt_latent, paste0('Liger_embed.adt.',rand_seed,'.txt'), #args$output_atac,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)
...
write.table(
  adt_latent, paste0('Liger_UMAPembed.adt.',rand_seed,'.txt'), #args$output_atac,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)


meta <- readRDS('../seurat/data/cite.rna.meta.rds')
write.table(meta, '../cite.rna.meta.csv',quote=F,sep=',')
