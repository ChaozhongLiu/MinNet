...
out_x = pd.read_csv('LigerINMF_embed.rna.%s.txt'%(randn),header=None).iloc[:,1:].to_numpy()
out_y = pd.read_csv('LigerINMF_embed.adt.%s.txt'%(randn),header=None).iloc[:,1:].to_numpy()
col_mean = np.nanmean(out_x, axis=0)
inds = np.where(np.isnan(out_x))
out_x[inds] = np.take(col_mean, inds[1])

col_mean = np.nanmean(out_y, axis=0)
inds = np.where(np.isnan(out_y))
out_y[inds] = np.take(col_mean, inds[1])
...
neighbor_1 = np.argsort(match_matrix_1, axis=1, kind='stable')
neighbor_1 = np.argsort(neighbor_1, axis=1, kind='stable')
writer = open('../results/raw/neighborhood_s4d1_LigerINMF.%s.txt'%(randn),'w')
...
writer = open('../results/raw/neighborhood_s4d8_LigerINMF.%s.txt'%(randn),'w')
...
writer = open('../results/raw/neighborhood_s4d9_LigerINMF.%s.txt'%(randn),'w')
...
#label transfer
meta = pd.read_csv('../cite.rna.meta.csv')
out_x = pd.read_csv('LigerINMF_embed.adt.%s.txt'%(randn),header=None).iloc[:,1:].to_numpy()
out_y = pd.read_csv('LigerINMF_embed.rna.%s.txt'%(randn),header=None).iloc[:,1:].to_numpy()
col_mean = np.nanmean(out_x, axis=0)
inds = np.where(np.isnan(out_x))
out_x[inds] = np.take(col_mean, inds[1])
...
pred_df.to_csv('../results/raw/LigerINMF_prediction.%s.txt'%(randn))
...
acc_df = pd.DataFrame({'cell_type':ct_heatmap.columns.tolist(),
                       'LigerINMF': np.diag(ct_heatmap)})
acc_df.to_csv('../results/raw/LigerINMF_transfer_acc.%s.csv'%(randn))
...
merge.to_csv('../results/raw/LigerINMF_embed.%s.txt'%(randn), index=False)
