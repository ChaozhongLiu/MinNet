...
out_x = pd.read_csv('../results/raw/Liger_UMAPembed.rna.%s.txt'%(randn),header=None)
out_y = pd.read_csv('../results/raw/Liger_UMAPembed.atac.%s.txt'%(randn),header=None)
...
neighbor_1 = np.argsort(match_matrix_1, axis=1, kind='stable')
neighbor_1 = np.argsort(neighbor_1, axis=1, kind='stable')
writer = open('../results/raw/neighborhood_s4d1_Liger.UMAP.%s.txt'%(randn),'w')
...
writer = open('../results/raw/neighborhood_s4d8_Liger.UMAP.%s.txt'%(randn),'w')
...
writer = open('../results/raw/neighborhood_s4d9_Liger.UMAP.%s.txt'%(randn),'w')
...
pred_df = pd.DataFrame({'cell_type':labels_x,'prediction':pred_y})
pred_df.to_csv('../results/raw/Liger_prediction.UMAP.%s.txt'%(randn))
...
acc_df = pd.DataFrame({'cell_type':ct_heatmap.columns.tolist(),
                       'Liger': np.diag(ct_heatmap)})
acc_df.to_csv('../results/raw/Liger_transfer_acc.UMAP.%s.csv'%(randn))
...
merge.insert(0, 'index', np.concatenate((meta.index.to_numpy(), 
                                         meta.index.to_numpy()), axis=0))
merge.to_csv('../results/raw/Liger_UMAPembed.%s.txt'%(randn), index=False)
