...
def feature_selection_multiome(input_mod1, input_mod2, path):
    gene_index = open('%s/gene_index.txt'%(path),'r').readlines()
    gene_index = [i.strip() for i in gene_index]
    gene_index = np.array(gene_index)
    #peak_index = open('%s/peak_index.txt'%(path),'r').readlines()
    #peak_index = [i.strip() for i in peak_index]
    ...
    intersect_gene = np.intersect1d(input_mod2.var_names.to_numpy(), gene_index)
    cpl_gene = np.setdiff1d(gene_index, input_mod2.var_names.to_numpy())
    if len(cpl_gene) > 0:
        print('Complementing missing feaures with 0')
        input_mod2 = input_mod2[:,intersect_gene]
        cpl_matrix = sp.sparse.csr_matrix((input_mod2.n_obs, len(cpl_gene)), dtype=np.float64)
        input_mod1_cpl = ad.AnnData(
            X=cpl_matrix,
        )

        input_mod1_cpl.var_names = cpl_gene
        input_mod1_cpl.obs_names = input_mod2.obs_names

        test = ad.concat([input_mod2, input_mod1_cpl], axis=1)
        test.obs = input_mod2.obs
        test.uns = input_mod2.uns
        test = test[:,gene_index]
        input_mod2 = test
    else:
        input_mod2 = input_mod2[:,gene_index]
