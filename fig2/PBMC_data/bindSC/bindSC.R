#setwd('/mnt/hdd/chaozhong/manuscript/fig2/PBMC_data/bindSC/')
#library(devtools)
...
rna <- readRDS('../data/pbmc.rna.rds')
atac <- readRDS('../data/pbmc.atac.rds')

rna.so <- CreateSeuratObject(counts = rna@assays$RNA@counts, 
                             meta.data = rna@meta.data)

atac.so <- CreateSeuratObject(counts = atac@assays$ATAC@counts, 
...
X.clst <- rna@meta.data$seurat_annotations
Y.clst <- atac@meta.data$seurat_annotations
