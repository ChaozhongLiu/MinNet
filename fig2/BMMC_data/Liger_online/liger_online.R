#setwd('/mnt/hdd/chaozhong/manuscript/fig2/BMMC_data/Liger_online/')
...
int.liger <- selectGenes(int.liger, datasets.use = 2, var.thresh = 0.2, do.plot = F)
int.liger <- scaleNotCenter(int.liger)


#---------------------------------
cat("[3/4] Running LIGER...\n")
int.liger <- online_iNMF(int.liger, k = 20, 
                         miniBatch_size = 5000,
                         max.epochs = 5,
                         seed = rand_seed)
...
write.table(
  rna_latent, paste0('../results/raw/LigerOnline_embed.rna.',rand_seed,'.txt'), #args$output_rna,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)
write.table(
  atac_latent, paste0('../results/raw/LigerOnline_embed.atac.',rand_seed,'.txt'), #args$output_atac,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)
...
rownames(atac_latent) <- gsub("\\.ATAC$", "", rownames(atac_latent))
write.table(
  rna_latent, paste0('../results/raw/LigerOnline_UMAPembed.rna.',rand_seed,'.txt'), #args$output_rna,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)
write.table(
  atac_latent, paste0('../results/raw/LigerOnline_UMAPembed.atac.',rand_seed,'.txt'), #args$output_atac,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)
