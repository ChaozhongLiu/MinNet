...
rna = anndata.read_h5ad("rna_preprocessed.s3s4.h5ad")
atac = anndata.read_h5ad("atac_preprocessed.s3s4.h5ad")
graph = nx.read_graphml("prior.graphml.s3s4.gz")
...
glue.save("glue_s3s4/final.%s.dill"%(randn))
...
merge.to_csv('../results/raw/GLUE_embed.s3s4.%s.txt'%(randn), index=False)
...
pred_df.to_csv('../results/raw/GLUE_prediction.s3s4.%s.txt'%(randn))
...
acc_df.to_csv('../results/raw/GLUE_transfer_acc.s3s4.%s.csv'%(randn))
