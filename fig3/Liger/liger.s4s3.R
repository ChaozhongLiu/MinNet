setwd('/mnt/hdd/chaozhong/manuscript/fig3/Liger/')
...
rna <- readRDS('../seurat/bmmc.rna.s4d1.rds')
atac <- readRDS('../seurat/bmmc.atac.s3d7.rds')
...
write.table(
  rna_latent, paste0('Liger_embed.rna.s4s3.',rand_seed,'.txt'), #args$output_rna,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)
write.table(
  atac_latent, paste0('Liger_embed.atac.s4s3.',rand_seed,'.txt'), #args$output_atac,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)
...
rownames(atac_latent) <- gsub("\\.ATAC$", "", rownames(atac_latent))
write.table(
  rna_latent, paste0('Liger_UMAPembed.rna.s4s3.',rand_seed,'.txt'), #args$output_rna,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)
write.table(
  atac_latent, paste0('Liger_UMAPembed.atac.s4s3.',rand_seed,'.txt'), #args$output_atac,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)
