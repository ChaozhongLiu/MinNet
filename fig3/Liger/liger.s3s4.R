...
rna <- readRDS('../seurat/bmmc.rna.s3d7.rds')
atac <- readRDS('../seurat/bmmc.atac.s4d1.rds')
...
write.table(
  rna_latent, paste0('Liger_embed.rna.s3s4.',rand_seed,'.txt'), #args$output_rna,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)
write.table(
  atac_latent, paste0('Liger_embed.atac.s3s4.',rand_seed,'.txt'), #args$output_atac,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)
...
rownames(atac_latent) <- gsub("\\.ATAC$", "", rownames(atac_latent))
write.table(
  rna_latent, paste0('Liger_UMAPembed.rna.s3s4.',rand_seed,'.txt'), #args$output_rna,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)
write.table(
  atac_latent, paste0('Liger_UMAPembed.atac.s3s4.',rand_seed,'.txt'), #args$output_atac,
  sep = ",", row.names = TRUE, col.names = FALSE, quote = FALSE, na = ""
)
