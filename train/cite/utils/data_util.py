...
class dataset(object):
    ...
    def init_dataset(self):
        ...
        peak_index = open('data/protein_index.txt','r').readlines()
        peak_index = [i.strip() for i in peak_index]
        peak_index = np.array(peak_index)
        self.input_mod2 = self.input_mod2[:,peak_index]
        ...
        self.input_mod2.obs['cell_type_name'] = self.input_mod1.obs['cell_type']
    ...
    def prepare_siamese(self):
        ...
        #self._use_domain_mod1 = np.zeros(self.num_train)
        #self._use_domain_mod2 = np.zeros(self.num_train)
...
class test_data(object):
    ...
    def init_dataset(self):
        ...
        print('Normalizing data...')
        sc.pp.scale(self.input_mod1, max_value=10) #, zero_center=False)
        sc.pp.scale(self.input_mod2, max_value=10) #, zero_center=False)
        self.input_mod1.X = sp.sparse.csr_matrix(self.input_mod1.X)
        self.input_mod2.X = sp.sparse.csr_matrix(self.input_mod2.X)
