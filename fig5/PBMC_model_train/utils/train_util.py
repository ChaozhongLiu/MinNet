...
from .function_util import split_loss, loss_hist
...
class Siamese_Trainer(BaseTrainer):
    def __init__(self, dataset, num_epochs, batch_number, model_path, num_peaks, num_genes,
                 margin=1, lamb=0.1, use_gpu=True, validation=True, lr=1e-3):
        #Setup network
        ...
        self.lamb = lamb
        self.lr = lr
        self.D = SiamAtt(num_peaks, num_genes, int(self.dataset.label_num)).cuda()
    ...
    def test(self):
        ...
        for i in range(n_iter):
            ...
            output_x, output_y, _, _ = self.D(X, Y, mode='train')
        ...
        #match_matrix = pairwise_distances(output_X, output_Y, metric="euclidean")
        #match_matrix = np.exp(-match_matrix)
        #match_matrix = match_matrix / np.sum(match_matrix, axis=1)[:, np.newaxis]
